...
int intersection_size(struct node *head1, struct node *head2) {
    ...
    for (struct node * list1_iter = head1; list1_iter != NULL; 
    list1_iter = list1_iter->next) {
        for (struct node * list2_iter = head2; list2_iter != NULL; 
        list2_iter = list2_iter->next) {
            //printf("%d-%d\n",list1_iter->data , list2_iter->data);
            if (list1_iter->data == list2_iter->data) {
                intersection_size++;
            }
        }
    }
