...
int sum_ast(int size); //calculate sum of asterisks
int initial_condition(int sum); /*calculate initial condition use no. * start 
with 0 and cycle */
int main(void) {
    int size;
    printf("Enter size: ");
    ...
    //printf("%d\n", initial_condition(sum_ast(size)));
    return 0;
}
void print_char(int col, int row, int size){
    int i = 0; //col
    ...
    */ 
    ...
    int left = 3;
    int up_ = 4;
    int dec_num = initial_condition(sum_ast(size));
    while (move <= size) {
        if (move == size ) {
...
void print_spiral(int size){
    ...
    int i = 0; //col
    int j = 0; //row
    //int count = initial_condition(sum_ast(size));
    //int sum = 0;
    while (j < size) {
        while (i < size) {
            print_char(i, j, size);
...
*/
int sum_ast(int size){

    int sum = 0;
    int count = 0;
    int size_sto = size;
 
    while (count <= size_sto) {
        sum = sum + size;
        if (count == 0) {
            size = size - 1;
        }
