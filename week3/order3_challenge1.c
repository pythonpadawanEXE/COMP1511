...
int min = (a < b && a < c)*a + (b < c && b < a)* b + (c < a && c < b) * c + (a == b && a < c)*a + (b == c && b < a)*b + (c == a && c < b)*c + (a == b && a == c)*a;
int mid = ((b <= a && a <= c) || (c <= a && a <= b)) * a + ((a <= b && b <= c) || (c <= b && b <= a)) * b + ((b <= c && c <= a) || (a <= c && c <= b)) * c -(a == c)* a -(a == b)* a - (b == c) * b + (a == b && b == c)*c ;
